...
public class AM_WorldGenCrypt extends WorldGenerator
{

    public AM_WorldGenCrypt()
    {
    }
	...
	world.setBlock(i+x,j+y,z+k,0);
		...
		world.setBlockWithNotify(i+(x-w2), j+height, k+(z-l2), 43);//Block.sandStone.blockID);
		}
		}*/
		
		for(int z=1;z<=length-1;z++){
		for(int x=1;x<=width-1;x++){
		world.setBlockAndMetadata(i+(x-(w2)), j+height+1, k+(z-(l2)), 44,1);
		}
		}
		int R=random.nextInt(8);
		if(R==1){
		world.setBlockWithNotify(i,j+1,k,AutomatonLogger.crystal);
		}else if(R==2){
		world.setBlockWithNotify(i,j+1,k,AutomatonLogger.heal);
		}else if(R==3){
		...
		world.setBlockWithNotify(i,j+1,k,AutomatonLogger.fakeCrystal);
		}
	}
	private void struct(World world,int O, int height,int width,int length, int i, int j, int k){
		...
		for(int x=0;x<=w3;x++){
		for(int y=0;y<=height;y++){
		world.setBlockWithNotify((x-w4)+i, j+y, k+l2, b);
		}
		}
		...
		for(int y=0;y<=height;y++){
		world.setBlockWithNotify(i+w2, j+y, k+(z-l2), b);
		}
		}
		...
		for(int y=0;y<height;y++){
		world.setBlockAndMetadataWithNotify(i-w2, j+y, k-l2, b,4);
		}
		for(int y=0;y<height;y++){
		world.setBlockAndMetadataWithNotify(i-w2, j+y, k+l2, b,4);
		}
	...
	/*
		...
		for(int y=0;y<=h3;y++){
		for(int x=0;x<=w3;x++){
		world.setBlockWithNotify(i+(x-w4), j+y+1, k+(z-l4), 0);
		}
		}
		}*/
		...
		for(int z=0;z<=l3;z++){
		for(int x=0;x<=w3;x++){
		world.setBlockWithNotify(i+(x-w4), j, k+(z-l4), Block.sandStone.blockID);
		}
		}
	...
	private void column(World world, int height, int i, int j, int k){
		...
		for(int z=-2;z<=2;z++){
		for(int y=0;y>=-height;y--){
			world.setBlockWithNotify(i+x, j+y, k+z, Block.cobblestoneMossy.blockID);
			}
		}
		}
