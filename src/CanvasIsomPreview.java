    ...
    implements KeyListener, MouseListener, MouseMotionListener, Runnable
{
    ...
    public void loadWorld(String s)
    {
        ...
        synchronized(imageBufferList)
        {
            ...
            for(int i = 0; i < 64; i++)
            {
                for(int j = 0; j < 64; j++)
                {
                    imageBuffers[i][j].setWorldAndChunkPosition(worldObj, i, j);
                }
    ...
    private void setTimeOfDay(int i)
    {
        synchronized(imageBufferList)
        {
            ...
            for(int j = 0; j < 64; j++)
            {
                for(int k = 0; k < 64; k++)
                {
                    imageBuffers[j][k].setWorldAndChunkPosition(worldObj, j, k);
                }
    ...
    private IsoImageBuffer getImageBuffer(int i, int j)
    {
        ...
        if(isoimagebuffer.chunkX == i && isoimagebuffer.chunkZ == j)
        {
        ...
        isoimagebuffer.setChunkPosition(i, j);
        return isoimagebuffer;
    }
