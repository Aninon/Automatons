...
public class AutomatonActions
{
	...
	public AutomatonActions()
	{
	}
	static int nerp[][];
	
	static int nerk[][];
public static void Frassify(World world,EntityPlayer entity){
	...
	public static void Frassify(World world,int i,int j,int k){
		...
		for(int n=0;n<nerp.length;n++){
			...
			world.setBlockAndMetadata(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],AutomatonLogger.frass2,1);
			}else if(h==12){
			world.setBlockAndMetadata(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],AutomatonLogger.frass,2);
			}else if(h==82){
			world.setBlockAndMetadata(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],AutomatonLogger.frass,0);
			}else if(h==18){
			world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],AutomatonLogger.crink);
			}else if(h==17){
			world.setBlockAndMetadata(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],AutomatonLogger.tech,1);
			}else if(h==2){
		...
		world.markBlocksDirty(i-r,j-r,k-r,i+r,j+r,k+r);
	...
	public static void Blockify(int block,World world,int i,int j,int k){
		for(int n=0;n<nerp.length;n++){
			...
			if(world.getBlockId(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2])!=0){
				world.setBlockWithNotify(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],block);
			}
			}
		...
		public static void Naturalization(World world,EntityPlayer entity){
	int i=MathHelper.floor_double(entity.posX);
	int j=MathHelper.floor_double(entity.posY);
	int k=MathHelper.floor_double(entity.posZ);
	...
	public static void Naturalization(World world,int i,int j,int k){
		for(int n=0;n<nerp.length;n++){
			...
			if(h==3||(h==AutomatonLogger.frass &&(m==0||m==3))){
				...
				world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],0);
			}else if(h==AutomatonLogger.crink){
				world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],18);
			}else if(h==AutomatonLogger.tech &&m==1){
			...
			switch(r){
				...
				case 1:world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],Block.plantYellow.blockID);break;
				case 2:world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],Block.plantRed.blockID);break;
				default:world.setBlock(i+nerp[n][0],j+nerp[n][1],k+nerp[n][2],Block.reed.blockID);break;
			}
			}
	...
	boolean cell[][][]=new boolean[D*2][D*2][D*2]
	
	
	
	for(int b=0;b<D;b++){
		int xx=(int)Math.round(vs.xCoord +vp.xCoord * ((double)b));
		int yy=(int)Math.round(vs.yCoord +vp.yCoord * ((double)b));
		int zz=(int)Math.round(vs.zCoord +vp.zCoord * ((double)b));
		count++;
	}
	
	int nerk[][]=new int[count][3];
	
	int co=0;
	for(int xi=0;xi<D*2;xi++){
	for(int yi=0;yi<D*2;yi++){
	...
	//System.out.println(vs.posY);
	//System.out.println(vp.yCoord);
	...
	//int xs=vs.xCoord
		...
		switch (h){
		case 1:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,0);break;
		case 2:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,1);break;
		case 3:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,6);break;
		case 4:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,2);break;
		case 9:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,3);break;
		case 11:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,4);break;
		case 82:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,5);break;
		case 12:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,7);break;
		case 24:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,8);break;
		case 13:world.setBlockAndMetadata(i+nerk[n][0],j+nerk[n][1],k+nerk[n][2], AutomatonLogger.hollow,9);break;
		}
			...
			for(int x=0;x<diam;x++){
				for(int z=0;z<diam;z++){
					if(FValue(x-size, y-size, z-size, size) <= 0) {
						...
						der.add(x-size);der.add(y-size);der.add(z-size);
						ar.add(der);
					}	
				}
			}
